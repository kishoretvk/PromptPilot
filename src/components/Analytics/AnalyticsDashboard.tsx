...
const AnalyticsDashboard: React.FC = () => {
  ...
  // Get the individual hooks from useAnalytics
  const { 
    useUsageMetrics: UsageMetricsHook,
    usePerformanceData: PerformanceDataHook,
    useCostData: CostDataHook,
    useDashboardData: DashboardDataHook
  } = useAnalytics();
  
  // Use the hooks to get data
  const { data: usageMetrics, isLoading: usageLoading } = UsageMetricsHook(timeRange);
  const { data: performanceData, isLoading: performanceLoading } = PerformanceDataHook(timeRange);
  const { data: costData, isLoading: costLoading } = CostDataHook(timeRange);
  const { data: analyticsData, isLoading: analyticsLoading, refetch: refetchAnalytics } = DashboardDataHook(timeRange);
  ...
  const usageChartData = {
    ...
    datasets: [
      ...
      {
        label: 'Tokens',
        data: usageMetrics?.usage_by_day?.map(item => item.executions) || [], // This should be tokens if available
  ...
  const providerData = {
    labels: usageMetrics?.top_providers?.map(item => item.provider) || [],
    datasets: [
      {
        data: usageMetrics?.top_providers?.map(item => item.executions) || [],
        backgroundColor: [
              ...
              <CardContent>
                {analyticsLoading ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {analyticsData?.top_prompts?.slice(0, 5).map((prompt: any, index: number) => (
            ...
            <Card>
              ...
              <CardContent>
                {analyticsLoading ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {analyticsData?.error_analysis?.slice(0, 5).map((error: any, index: number) => (
                      <Box key={index} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Card>
              ...
              <CardContent>
                {performanceLoading ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {performanceData?.provider_performance?.map((provider: any, index: number) => (
                      <Box key={index}>
